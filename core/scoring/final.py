...
def compute_final_score_with_patterns(
    ...
    try:
        ...
        ml_val = np.nan
    if pd.notna(ml_val):
        if ml_val < float(ML_GATES.get("penalty_lt", 0.15)):
            multiplier = float(ML_GATES.get("penalty_mult", 0.60))
        elif ml_val > float(ML_GATES.get("bonus_gt", 0.62)):
            multiplier = float(ML_GATES.get("bonus_mult", 1.15))
