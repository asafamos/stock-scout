...
def build_all_ml_features_v4(
    ...
    """Build 30-feature vector for V4.0 model.

    V4.0 = 19 retained technical (from v3.6)
          + 7 fundamental (ROE, PE, Revenue, EPS, D/E, Beta, MarketCap)
          + 4 alternative/interaction (Insider, Analyst PT, Earnings, Quality×Momentum)

    Fundamental data comes from the scan row (populated by core/row_builder.py).
    Alternative data is fetched live from core/sentiment_data.py.

    Args:
        row: Series with technical indicators + fundamental fields
             (ROE_f, PE_f, RevG_f, EPSG_f, DE_f, Beta, MarketCap)
        df_hist: Historical OHLCV DataFrame
        market_context: Optional market regime/SPY data
        sector_context: Optional sector relative data

    Returns:
        Dict with all 30 V4.0 features.
    """
    # Start with all V3.6 features (23) — we'll select the 19 we need
    all_v36 = build_all_ml_features_v3_6(row, df_hist, market_context, sector_context)

    # Helper to safely extract float from row
    def _safe(key: str, default: float = np.nan) -> float:
    ...
    # Select only the features that are in v4 from the v3.6 output
    v4_technical_names = [
        "Support_Strength", "Distance_From_52w_Low", "Consolidation_Tightness",
        "Distance_To_Resistance", "Sector_Momentum", "Sector_Rank",
        "RS_vs_SPY_20d", "Return_20d", "Momentum_Consistency",
        "Tightness_Ratio", "ATR_Pct", "VCP_Ratio", "Volume_Surge",
        "RSI_Delta_5d", "ATR_Delta_5d", "Volume_Delta_5d",
        "Momentum_Acceleration", "VCP_x_RS", "Momentum_x_Volume",
    ]
    result: Dict[str, float] = {}
    for name in v4_technical_names:
        result[name] = all_v36.get(name, 0.0)

    # ── 7 fundamental features ──
    # ROE z-score (using population stats from training)
    roe_raw = _safe("ROE_f", np.nan)
    if not np.isnan(roe_raw):
        roe_z = (roe_raw - _FUNDAMENTAL_STATS["ROE_mean"]) / max(_FUNDAMENTAL_STATS["ROE_std"], 1e-6)
        result["ROE_Zscore"] = float(np.clip(roe_z, -3, 3))
    else:
        result["ROE_Zscore"] = 0.0

    # PE rank (inverted: low PE = high rank, using median as reference)
    pe_raw = _safe("PE_f", np.nan)
    if not np.isnan(pe_raw) and pe_raw > 0:
        # Simple sigmoid-based rank: PE near median → 0.5, low PE → 1.0, high PE → 0.0
        pe_ratio = pe_raw / max(_FUNDAMENTAL_STATS["PE_median"], 1.0)
        result["PE_Rank"] = float(np.clip(1.0 / (1.0 + pe_ratio), 0, 1))
    else:
        result["PE_Rank"] = 0.5

    # Revenue growth
    result["Revenue_YoY"] = float(np.clip(_safe("RevG_f", 0.0), -0.5, 2.0))

    # EPS growth
    result["EPS_Growth_YoY"] = float(np.clip(_safe("EPSG_f", 0.0), -1.0, 3.0))

    # Debt/Equity z-score (inverted: low D/E = positive z-score)
    de_raw = _safe("DE_f", np.nan)
    if not np.isnan(de_raw):
        de_z = -(de_raw - _FUNDAMENTAL_STATS["DE_mean"]) / max(_FUNDAMENTAL_STATS["DE_std"], 1e-6)
        result["Debt_Equity_Zscore"] = float(np.clip(de_z, -3, 3))
    else:
        result["Debt_Equity_Zscore"] = 0.0

    # Beta
    result["Beta"] = float(np.clip(_safe("Beta", 1.0), 0.2, 3.0))

    # Log market cap
    mcap = _safe("MarketCap", 1e10)
    result["Log_MarketCap"] = float(np.clip(np.log(max(mcap, 1e6)), 18, 28))

    # ── 4 alternative/sentiment features ──
    ticker = ""
    if hasattr(row, 'get'):
        ticker = row.get("ticker", "") or row.get("Ticker", "") or row.get("symbol", "")
    elif hasattr(row, 'name'):
        ticker = str(row.name) if row.name else ""

    # Fetch alternative data (uses caching internally, ~100ms per call)
    try:
        from core.sentiment_data import (
            fetch_insider_trades_finnhub,
            fetch_analyst_ratings_fmp,
            fetch_earnings_data_finnhub,
        )

        if ticker:
            insider = fetch_insider_trades_finnhub(ticker)
            result["Insider_Net_30d"] = float(np.clip(
                insider.get("insider_net_30d", 0.0), -1, 1
            ))

            analyst = fetch_analyst_ratings_fmp(ticker)
            result["Price_Target_Upside"] = float(np.clip(
                analyst.get("price_target_upside", 0.1), -0.5, 1.0
            ))

            earnings = fetch_earnings_data_finnhub(ticker)
            days_to = earnings.get("days_to_earnings", 45)
            result["Days_To_Earnings_Norm"] = float(np.clip(days_to / 90.0, 0, 1))
        else:
            result["Insider_Net_30d"] = 0.0
            result["Price_Target_Upside"] = 0.1
            result["Days_To_Earnings_Norm"] = 0.5
    except Exception:
        result["Insider_Net_30d"] = 0.0
        result["Price_Target_Upside"] = 0.1
        result["Days_To_Earnings_Norm"] = 0.5
    ...
    result["Quality_x_Momentum"] = float(np.clip(
        result["ROE_Zscore"] * result["Momentum_Consistency"], -3, 3
    ))
